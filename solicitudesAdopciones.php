...
$qSolicitudes = "SELECT folio_solicitud, s.idSolicitud, numSolicitud, date_format(fecha_entrega_solicitud, '%d/%m/%Y') as fecha_sol, hi.estado, group_concat(' ',concat(u.nombre, ' ', u.apellido_p, ' ', u.apellido_m)) as solicitantes
	...
	inner join solicitantes_adop so on so.id_solicitud=s.idSolicitud
	inner join usuarios u on u.id=so.id_usuario
	group by s.idSolicitud";

$solicitudes = $mysqli->query($qSolicitudes);
    ...
    <div id="wrapper">
        <div id="main">
            <div class="inner">
                ...
                <h2>Expedientes de solicitantes de adopción</h2>

                <form>
                    <input type="search" name="palabra" id="query" style="text-transform:uppercase;"
                        placeholder="Folio de la solicitud o nombre de algun solicitante" /><br>
                    <table>
                        <thead>
                            <tr>
                                <th>Folio</th>
                                ...
                                <th>Nombre de los solicitantes</th>
                                <th>Estatus</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if ($solicitudes->num_rows > 0) {
    while ($rw = $solicitudes->fetch_assoc()) {?>
                            <tr>
                                <?php if ($id == 56 or $id == 63 or $id == 62 or $id == 48 or $id == 23 or $id == 234 or $id == 228) {?>

                                <td><a
                                        href="perfilSolAdop.php?id=<?=$rw['idSolicitud']?>"><?=$rw['folio_solicitud']?></a>
                                </td>
                                <?php } else {?>
                                <td><?=$rw['folio_solicitud']?></td>
                                <?php }?>
                                <td><?=$rw['numSolicitud']?></td>
                                <td><?=$rw['fecha_sol']?></td>
                                <td><?=$rw['solicitantes']?></td>
                                <td><?=$rw['estado']?></td>
                            </tr>
                            <?php }
} else {
    echo "<tr><th colspan='5'>NO HAY REGISTROS AÚN</th></tr>";
}
?>
                        </tbody>
                    </table>
