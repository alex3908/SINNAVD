<?php	
	...
	$sql="SELECT reportes_vd.folio, reportes_vd.fecha, reportes_vd.recepcion, distritos.distrits, tipo_maltrato.maltrato, 
	reportes_vd.nom_nna, reportes_vd.edad_nna, reportes_vd.fn_nna, reportes_vd.lugarnac_nna, reportes_vd.lugarreg_nna, 
	reportes_vd.otros_datos, reportes_vd.persona_reporte, reportes_vd.narracion, municipios.municipio, localidades.localidad, 
	reportes_vd.calle, reportes_vd.ubicacion, departamentos.responsable, reportes_vd.asignado, 
	reportes_vd.asignado_psic, reportes_vd.asignado_j, reportes_vd.atendido, reportes_vd.respo_reg 
	from reportes_vd
	left join  municipios on municipios.id=reportes_vd.clm 
	left join departamentos on departamentos.id=reportes_vd.respo_reg
	left join tipo_maltrato on tipo_maltrato.id=reportes_vd.id_maltrato
 	left join localidades on localidades.id=reportes_vd.id_localidad  
 	left join distritos  on distritos.id=reportes_vd.id_distrito
	where reportes_vd.id='$idReporte'";  //recupera los datos del reporte
	...
	$val="SELECT reportes_vd.id from reportes_vd, part1ac where reportes_vd.id='$idReporte' and part1ac.id_reporte=reportes_vd.id";
	$eval=$mysqli->query($val);
	$siHAY=$eval->num_rows;

	if (isset($_POST['eliminar'])) {
		$eli="DELETE from reportes_vd where id='$idReporte'";
		$eeli=$mysqli->query($eli);
		header("Location: lista_reporte.php?estRep=0");
	}
...
<html>
	...
	<?php while ($row=$esql->fetch_assoc()) { ?>	
						...
						<div class="row uniform">
							...
							<div class="2u$(xsmall)">
								<?php if ($siHAY>0) { ?>
							<input type="submit" class="button special small" name="eliminar" value="eliminar" disabled>		
								<?php } else { ?>
	...
	<table>
		<caption>Asignados</caption>
			<thead>
				<tr>
					<td>Juridico: <?php if (is_null($resj)) { if ($rows2=='0') { ?> sin asignar
							...
							<?php } }else{ echo $resj; if($_SESSION['departamento']==16 or $idDEPTO==223){ ?> <!--solo control de información y erika en su cuenta de administrativo-->
					...
					<td>Trabajo Social: <?php if (is_null($res)) { if ($rows2=='0') { ?> sin asignar
							...
							<?php } }else{ echo $res; if($_SESSION['departamento']==16 or $idDEPTO==223){ ?>
					...
					<td>Psicologia: <?php if (is_null($resp)) { if ($rows2=='0') { ?> sin asignar
							...
							<?php } }else{ echo $resp;if($_SESSION['departamento']==16 or $idDEPTO==223){ ?>
			...
			<?php $atendido=$row['atendido'];	  $respoR=$row['respo_reg'];
										...
										if ($idD=='16' and $idP='1') { //administrador (mueve a cualquier estado)?>
						<table class="alt">									
							<thead>
								<tr>
									<?php if ($atendido==1) { ?>
...
<th><input type="radio" id="AP" name="demo-priority" value="4" onchange="this.form.submit()" checked disabled>
				...
				<?php }else if ($respoR=$idDEPTO) { //respo de reg reporte (cambia a los estados de adelante c/s acercamientos) ?>
						<table class="alt">									
							<thead>
								<tr>
									<?php if ($atendido==1) { ?>
...
<th><input type="radio" id="AP" name="demo-priority" value="4" checked disabled>
				...
				<?php }else if ($asig==$idDEPTO or $asigp==$idDEPTO or $asigj=$idDEPTO) { //si es tu reporte (avanza de acuerdo a acercamientos)?>
						<table class="alt">									
							<thead>
								<tr>
									<?php if ($atendido==1) { ?>
...
<th><input type="radio" id="AP" name="demo-priority" value="4" onchange="this.form.submit()" checked disabled>
									...
									<?php } ?>																					
										...
										<?php }else { //no es tu reporte (No lo cambia)?>
						<table class="alt">									
							<thead>
								<tr>
									<?php if ($atendido==1) { ?>
...
<th><input type="radio" id="AP" name="demo-priority" value="4" checked disabled>
						...
						<?php if ($asig>0 or $asigp>0) { 
							...
							if ($can>0) { ?>		<!--si can devuelve un valor el boton direcciona a acercmientos.php, y muestra datos-->					
							...
							<?php  }else {  //si el valor es null direcciona a num_nna_ac para registrar el num de nna
							...
							<div class="12u$">
								<div class="box">
									<ul class="alt">
										...
										<li><h4>Ubicación: </h4><?php echo 'Municipio '.$row['municipio'].'<br>Localidad '.$row['localidad'].'<br>Calle '.$row['calle'].'<br>Referencias '.$row['ubicacion'];  ?> </li>
						...
						</div> <br> <!--muestra datos del reporte-->
							<?php } ?>
