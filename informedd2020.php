 <?php	
	...
	$idDEPTO = $_SESSION['id'];	
	$anio = $_SESSION['anio'];
	$qmes="SELECT id, mes, fechas from cortes where año='$anio'"; 
		...
		<div id="wrapper">
			...
			<div id="main">
				<div class="inner">
					...
					<?php  if(!empty($_POST['ml'])){  //Boton municipios y loalidades
						...
						if (empty($fec)) {
							...
							$pf="SELECT fechai, fechac, fechas, mes from cortes where idXaño=$fec and año='$anio'";
							$epf=$mysqli->query($pf);
					...
					if(!empty($_POST['poba'])){
						...
						sexo.sexo, date_format(usuarios.fecha_nacimiento, '%d/%m/%Y %H:%i:%s') as fecha_reg 
						...
						// if($idDEPTO==220) echo $pf.";".$primera;?>
						<div class="box">
							<h4><?php echo $me; ?></h4>
					...
					if (empty($fec)) {
						...
						nna.apellido_m, nna.nombre, nna.curp, date_format(nna.fecha_nacimiento,'%d/%m/%Y') as fecha_nac, nna.sexo, 
						date_format(nna.fecha_registro, '%d/%m/%Y %H:%i:%i') as fecha_reg
						from nna left join municipios on municipios.id=nna.municipio
						left join localidades on localidades.id=nna.localidad
						where nna.activo=1 and nna.fecha_registro between '$fechai' and '$fechac' 
					...
					if (empty($fec)) {
						...
						$primera="SELECT distinct municipios.municipio, localidades.localidad, nna.id, nna.apellido_p, nna.apellido_m, nna.nombre, nna.curp, nna.sexo, 
						...
						order by municipio, localidad, nna.id";				
						$segunda="SELECT distinct municipios.municipio, localidades.localidad, nna.id, nna.apellido_p, nna.apellido_m, nna.nombre, date_format(nna.fecha_nacimiento, '%d/%m/%Y') as fecha_nac, 
						nna.curp, nna.sexo, nna.fecha_registro
						from nna left join localidades on localidades.id=nna.localidad 
						left join municipios on municipios.id=nna.municipio 
						left join benefmed on benefmed.id_nna=nna.id 
						...
						and (nna.fecha_registro not BETWEEN '$feci'  and '$fecc') order by municipio, localidad, nna.id";
						$eprimera=$mysqli->query($primera);
					...
					if (empty($fec)) {
						...
						$pf="SELECT fechas, fechai, fechac, mes from cortes where idXaño=$fec and año='$anio'";
						$epf=$mysqli->query($pf);
						while ($row=$epf->fetch_assoc()) {
								$feci=$row['fechai'];
								$fecc=$row['fechac'];
				...
				if (empty($fec)) {
					...
					where carpeta_inv.fecha_inicio between '$feci' and '$fecc' order by  municipios.id, delito";			
...
<?php }
	...
	case 4:  //c1 a4
		...
		$sql="SELECT municipios.municipio,  count(historial.id) as total
		...
		where historial.fecha_ingreso between '$feci' and '$fecc'
		and ( historial.atencion_brindada like '%ORIENTACION JURIDICA%') 
		and historial.asunto='INICIAL' 
		group by usuarios.id_mun having count(historial.id)";	
		$esql=$mysqli->query($sql); 
	$total="SELECT count(historial.id) as total from usuarios inner join historial on usuarios.id=historial.id_usuario where historial.fecha_ingreso between '$feci' and '$fecc' and ( historial.atencion_brindada like '%ORIENTACION JURIDICA%') and historial.asunto='INICIAL'";
	$total = $mysqli->query($total);
	$ev = implode($total->fetch_assoc());
	$qjuridicasL="SELECT date_format(fecha_ingreso, '%d/%m/%Y') as fecha,
		usuarios.id, direccion, departamentos.responsable
		from usuarios inner join historial on usuarios.id=historial.id_usuario
		inner join departamentos on departamentos.id=usuarios.respo_reg          
		and fecha_ingreso between '$feci' and '$fecc'
		and ( historial.atencion_brindada like '%ORIENTACION JURIDICA%') 
...
case 7:  //c1 a8
	$fec=$_POST['mess'];
		...
		$pf="SELECT  mes, fechai, fechac from cortes where idXaño=$fec and año='$anio'";
		$epf=$mysqli->query($pf);
		while ($row=$epf->fetch_assoc()) {
			$feci=$row['fechai'];
			$fecc=$row['fechac'];
...
case 11:  //c1 a26
	$fec=$_POST['mess'];
		...
		where nna.activo=1 and nna.fecha_registro BETWEEN '$feci' and '$fecc' and migrante=1 group by municipios.municipio";
		$esql2=$mysqli->query($sql2); ?>
		<div class="box">
			<h4><?php echo $me; ?></h4>
