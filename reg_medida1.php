<?php
	...
	$sqlbeneE="SELECT nna_exposito.id, nna_exposito.folio, nna_exposito.sexo, nna_exposito.fecha_reg, municipios.municipio, departamentos.responsable, nna_caso.estado from nna_exposito, municipios, departamentos, nna_caso where nna_caso.id_caso='$idCaso' and nna_caso.id_nna=nna_exposito.id and nna_caso.estado='E' and departamentos.id=nna_exposito.respo_reg and municipios.id=nna_exposito.municipio_deteccion";
	$esqlbeneE=$mysqli->query($sqlbeneE);
	...
	if(!empty($_POST))
	{
		...
		for ($i=0; $i <count($beneficiario) ; $i++) { 
			...
			$bene2=trim($bene2, ':');
		$sqlNino = "INSERT INTO cuadro_guia (id_caso, id_derecho, id_medida, marco, med_prot, beneficiario, responsable_med, atp_encargada, periodicidad, estado, fecha_eje, fecha, id_sp_registro) VALUES ('$idCaso', '$derecho', '$medida', '$marco', '$med_prot', '$bene2', '$responsable_med', '$atp_encargada', '$periodicidad', '0', '', '$fec', '$idDEPTO')";
	...
	<body>
		...
		<!-- Wrapper -->
			...
			<div class="row 10% uniform">
                    ...
                    <div class="6u 12u$(xsmall)">MEDIDA DE PROTECCIÓN ESPECIAL
                    <textarea name="med_prot" cols="" rows="" style="text-transform:uppercase;" onkeyup="this.value=this.value.toUpperCase();" required></textarea>
                    </div>
                    <div class="6u 12u$(xsmall)">BENEFICIARIO
                   						<div class="select-wrapper">
											<select name="beneficiario[]" multiple="multiple" size="5" style="height:70px; width:100%"  required>
												<?php while ($row=$esqlbene->fetch_assoc()) { ?>
													<option value="<?php echo $row['id']." ".$row['estado'];?>"><?php echo $row['nombre']." ".$row['apellido_p']." ".$row['apellido_m'];?></option>
												<?php }  ?>
												<?php while ($row=$esqlbeneE->fetch_assoc()) { ?>
													<option value="<?php echo $row['id']." ".$row['estado'];?>"><?php echo $row['folio']." ".$row['sexo'];?></option>
												<?php }  ?>
											</select>
										</div>
                    </div>
                    <div class="6u 12u$(xsmall)">INSTITUCIÓN O PERSONA RESPONSABLE
                    <input name='responsable_med' type='text' style="height:70px" style="text-transform:uppercase;" onkeyup="this.value=this.value.toUpperCase();" required>
                    </div>
                    ...
                    <div class="6u 12u$(xsmall)">PERIODICIDAD
                    ...
                    <div class="6u 12u$(xsmall)">RESPONSABLE DE REGISTRO
