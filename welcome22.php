	...
	$listaUsuariosNull="SELECT historial.id, historial.id_usuario, usuarios.nombre, usuarios.apellido_p, usuarios.apellido_m, depto.departamento, historial.asunto, date_format(historial.fecha_ingreso, '%d/%m/%Y %H:%i:%s') as fecha_ingreso, historial.fecha_salida, historial.responsable FROM historial inner join usuarios on usuarios.id=historial.id_usuario inner join depto on depto.id=historial.id_departamento WHERE historial.fecha_salida is null";
	$rUsuariosNull=$mysqli->query($listaUsuariosNull);
	$listaUsuariosNull2="SELECT historial.id, historial.id_usuario, usuarios.nombre, usuarios.apellido_p, usuarios.apellido_m, depto.departamento, historial.asunto, date_format(historial.fecha_ingreso, '%d/%m/%Y %H:%i:%s') as fecha_ingreso, historial.fecha_salida, historial.responsable FROM historial inner join usuarios on usuarios.id=historial.id_usuario inner join depto on depto.id=historial.id_departamento inner join departamentos departamentos.id=historial.responsable WHERE historial.fecha_salida is null";
	$rUsuariosNull2=$mysqli->query($listaUsuariosNull2);
	}else if($cargo==3){ //administrativo

	
$listaUsuariosNull="SELECT historial.id, historial.id_usuario, usuarios.nombre, usuarios.apellido_p, usuarios.apellido_m, depto.departamento, historial.asunto, DATE_FORMAT(historial.fecha_ingreso,'%d/%m/%Y %H:%i:%s') as fecha_ingreso, historial.fecha_salida, departamentos.responsable FROM historial inner join usuarios on usuarios.id=historial.id_usuario inner join depto on depto.id=historial.id_departamento inner join departamentos on historial.responsable=departamentos.id WHERE historial.id_departamento=$mi && historial.fecha_salida is null;";
	$rUsuariosNull=$mysqli->query($listaUsuariosNull);
	$rUsuariosNull2=$mysqli->query($listaUsuariosNull);
	}else if($cargo==6){ //unidad

	
$listaBeneficiariosNull="SELECT visitas_unidad.id, visitas_unidad.id_benef, visitas_unidad.responsable, visitas_unidad.tipo, visitas_unidad.asunto, visitas_unidad.fecha, benef_unidad.folio, benef_unidad.nombre, benef_unidad.apellido_p, benef_unidad.apellido_m, cat_asuntos_unidad.asunto FROM visitas_unidad inner join benef_unidad on visitas_unidad.id_benef=benef_unidad.id inner join cat_asuntos_unidad on visitas_unidad.asunto=cat_asuntos_unidad.id";
	$rBeneficiariosNull=$mysqli->query($listaBeneficiariosNull);
	$rBeneficiariosNull2=$mysqli->query($listaBeneficiariosNull);

}else { //atencion a usuarios 
	
$listaUsuariosNull="SELECT historial.id, historial.id_usuario, usuarios.nombre, usuarios.apellido_p, usuarios.apellido_m, depto.departamento, historial.asunto, historial.fecha_ingreso, historial.fecha_salida, historial.responsable FROM historial inner join usuarios on usuarios.id=historial.id_usuario inner join depto on depto.id=historial.id_departamento WHERE historial.id_departamento=$mi && historial.fecha_salida is null AND ( historial.responsable=$idDEPTO OR historial.responsable=0)";
	$rUsuariosNull=$mysqli->query($listaUsuariosNull);
	...
	<body>
		...
		<!-- Wrapper -->
			...
			<div class="row 10% uniform">
					...
					<div class="3u"><img src="images/dif.jpg" width="45px" height="75px" onclick="location='reporte1ra.php'"/></div>
				...
				<div class="box">
					...
					<div class="uniform row">
						...
						<div class="4u"><strong>Periodo:</strong> <?php echo $row['fechai']." - ".$row['fechac']; ?></div>
		...
		<div class="row uniform">
				<div class="3u 12u$(xsmall)"><center>
				...
				<?php }   ?></center>
							...
							<h3 align="center">BIENVENIDO(A): <a href="informe_personal.php"> <?php  echo $row['responsable'];} ?></a><br><?php }while ($row=$care->fetch_assoc()) {
								echo $row['personal'];
							} ?></h3>
								...
								<div class="box">
									<?php if($cargo==6){ ?> <!--mostrar para UIENNAVD-->
											...
											<td><?php echo $row['id_usuario'];?></td>
												...
												if ($mi==7 OR $mi==16) { ?>
												 ...
												 <?php }else if($cargo==3 or $cargo==5) {  //administrativo y subprocu
												 			...
												 			<td><input type="button" class="button special fit small" name="Terminar visita" value="Canalizar visita" onclick="location='canalizar_visita.php?id=<?php echo $row['id'];?>'">
												 		</td>
												<?php  }else if($row['responsable']==$nom) {?>
															<td><input type="button" class="button special fit small" name="Terminar visita" value="Terminar visita" onclick="location='terminar_visita.php?id=<?php echo $row['id'];?>'">
												 		</td>	 <?php }else {	?> 
