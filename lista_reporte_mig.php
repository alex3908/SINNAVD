...
<html>
		...
		<table class="alt">
			<thead>
				<tr>
					...
					<td><input type="button" value="alta" onclick="location='reg_reporte_migrantes.php'" class="button special"></td>
				</tr>
				<tr>
					<td colspan="2"><form id="buscador" name="buscador" method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>" onSubmit="return validarForm(this)">
						...
						<h4 onclick="location='lista_reporte_mig.php?estRep=0'">Total de reportes: <?php echo $rows3; ?></h4></form></td>
				</tr>
			</thead>
		</table>
			...
			<table >			
				<tr>
					<td><b>FOLIO</b></td>
					<td><b>FECHA</b></td>					
					<td><b>NNA MIGRANTE</b></td>
					<td><b>PAÍS DE ORIGEN</b></td>
	...
	if ($estRep==0) {
	if (empty($buscar)) {
		$query="SELECT reportes_migrantes.id, reportes_migrantes.folio, reportes_migrantes.fecha_reg, reportes_migrantes.persona_reporta, nna_resporte.id_nna, nna_resporte.id_reporte, nna_reporte.tipo_reporte, nna.id, nna.nombre, nna.apellido_p, nna.apellido_m, nna.pais_origen, d1.responsable as d1res, d2.responsable as d2res, from reportes_migrantes, nna_reporte, nna INNER JOIN  departamentos d1 ON reportes_vd.asignado=d1.id INNER JOIN departamentos d2 ON reportes_vd.asignado_psic=d2.id  order by reportes_vd.id desc limit 20";
	} else {
		$query="SELECT reportes_vd.id, reportes_vd.folio, reportes_vd.fecha, reportes_vd.nom_nna, reportes_vd.recepcion, reportes_vd.persona_reporte, d1.responsable as d1res, d2.responsable as d2res, reportes_vd.atendido, reportes_vd.ubicacion from reportes_vd INNER JOIN  departamentos d1 ON reportes_vd.asignado=d1.id INNER JOIN departamentos d2 ON reportes_vd.asignado_psic=d2.id where reportes_vd.folio like '%$buscar%' OR reportes_vd.fecha like '%$buscar%' OR reportes_vd.recepcion like '%$buscar%' OR reportes_vd.persona_reporte like '%$buscar%' OR d1.responsable like '%$buscar%'or d2.responsable like '%$buscar%' or reportes_vd.nom_nna like '%$buscar%' or reportes_vd.ubicacion like '%$buscar%' order by reportes_vd.id desc";
	}
	}else if ($estRep==5) {
	if (empty($buscar)) {
		$query="SELECT reportes_vd.id, reportes_vd.folio, reportes_vd.fecha, reportes_vd.nom_nna, reportes_vd.recepcion, reportes_vd.persona_reporte, d1.responsable as d1res, d2.responsable as d2res, reportes_vd.atendido, reportes_vd.ubicacion from reportes_vd INNER JOIN  departamentos d1 ON reportes_vd.asignado=d1.id INNER JOIN departamentos d2 ON reportes_vd.asignado_psic=d2.id where reportes_vd.asignado=0 order by reportes_vd.id desc limit 20 ";
	} else {
	...
	if (empty($buscar)) {
		$query="SELECT reportes_vd.id, reportes_vd.folio, reportes_vd.fecha, reportes_vd.nom_nna, reportes_vd.recepcion, reportes_vd.persona_reporte, d1.responsable as d1res, d2.responsable as d2res, reportes_vd.atendido, reportes_vd.ubicacion from reportes_vd INNER JOIN  departamentos d1 ON reportes_vd.asignado=d1.id INNER JOIN departamentos d2 ON reportes_vd.asignado_psic=d2.id where reportes_vd.atendido='$estRep' and reportes_vd.asignado!=0 order by reportes_vd.id desc limit 20";
	} else {
		$query="SELECT reportes_vd.id, reportes_vd.folio, reportes_vd.fecha, reportes_vd.nom_nna, reportes_vd.recepcion, reportes_vd.persona_reporte, d1.responsable as d1res, d2.responsable as d2res, reportes_vd.atendido, reportes_vd.ubicacion from reportes_vd INNER JOIN  departamentos d1 ON reportes_vd.asignado=d1.id INNER JOIN departamentos d2 ON reportes_vd.asignado_psic=d2.id where reportes_vd.atendido='$estRep' and reportes_vd.asignado!=0 and (reportes_vd.folio like '%$buscar%' OR reportes_vd.fecha like '%$buscar%' OR reportes_vd.recepcion like '%$buscar%' OR reportes_vd.persona_reporte like '%$buscar%' OR d1.responsable like '%$buscar%' or d2.responsable like '%$buscar%' or reportes_vd.nom_nna like '%$buscar%' or reportes_vd.ubicacion like '%$buscar%') order by reportes_vd.id desc";
	}
	}
 ...
 while($row=$resultado->fetch_assoc()){ ?>
						...
						<tr>
							<td><a href="perfil_reporte.php?id=<?php echo $row['id'];?>"><?php echo $row['folio'];?></a></td>
							...
							<td>
								...
								if (is_null($id_asi)) { ?>
										...
										<?php } }else{ ?>
										<input type="button" class="special button fit small" name="Asignar" value="No asignado" onclick="location='perfil_reporte.php?id=<?php echo $row['id'];?>'">
