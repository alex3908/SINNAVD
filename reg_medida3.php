<?php
	...
	if(!empty($_POST))
	{
		...
		$sqlNino = "INSERT INTO cuadro_guia (id_caso, id_derecho, id_medida, marco, med_prot, responsable_med, atp_encargada, periodicidad, estado, fecha_eje, fecha, id_sp_registro) VALUES ('$idCaso', '$derecho', '$medida', '$marco', '$med_prot', '$responsable_med', '$atp_encargada', '$periodicidad', '0', '', '$fec', '$idDEPTO')";
                   						...
                   						<div class="select-wrapper">
											<select name="beneficiario[]" multiple="multiple" size="5" style="height:70px; width:100%"  required>
												...
												<?php while ($row=$esqlbeneE->fetch_assoc()) { ?>
													<option value="<?php echo $row['id'];?>"><?php echo $row['folio']." ".$row['sexo'];?></option>
												<?php }  ?>
											</select>
										</div>
                    </div>
