...
<html>
	...
	<div class="box"> 
		...
		<?php 
				...
				$ind2="SELECT usuarios.id, usuarios.nombre, usuarios.apellido_p, usuarios.apellido_m, historial.atencion_brindada, historial.fecha_ingreso, historial.fecha_salida from historial inner join usuarios on usuarios.id=historial.id_usuario where historial.fecha_ingreso BETWEEN '$feci' and '$fecc'and historial.responsable in ($idRespo) and historial.fecha_salida is not null";				
				...
				from derechos_nna, casos, benefmed, cuadro_guia, nna, medidas, catalogo_medidas 
				...
				and cuadro_guia.id=benefmed.id_medida and benefmed.id_nna=nna.id and casos.id=cuadro_guia.id_caso 
				and derechos_nna.id=cuadro_guia.id_derecho and medidas.id=cuadro_guia.id_medida 
				and catalogo_medidas.id=cuadro_guia.id_mp and cuadro_guia.activo=1";
				...
				date_format(cuadro_guia.fecha_registro, '%d/%m/%Y') as fecha, cuadro_guia.estado 
				from derechos_nna, casos, benefmed, cuadro_guia, nna, medidas, catalogo_medidas 
				where cuadro_guia.id_medida in('03') and cuadro_guia.estado=1 and cuadro_guia.id_sp_registro in($idRespo) 
				...
				$ind17="SELECT casos.id, casos.folio_c, derechos_nna.derecho, medidas.medida_p, catalogo_medidas.medidaC,
				 ...
				 cuadro_guia.estado from derechos_nna, casos, benefmed, cuadro_guia, nna, medidas, catalogo_medidas
				  where cuadro_guia.id_medida in('01') and cuadro_guia.estado=1 and cuadro_guia.id_sp_registro in($idRespo) 
				  and (cuadro_guia.fecha_ejecucion BETWEEN '$feci' and '$fecc') 
				  and cuadro_guia.id=benefmed.id_medida and benefmed.id_nna=nna.id 
				  and casos.id=cuadro_guia.id_caso and derechos_nna.id=cuadro_guia.id_derecho
				   and medidas.id=cuadro_guia.id_medida 
				  and catalogo_medidas.id=cuadro_guia.id_mp and cuadro_guia.activo=1";
				...
				from seguimientos left join cuadro_guia on seguimientos.id_med=cuadro_guia.id 
				left join catalogo_medidas on catalogo_medidas.id=cuadro_guia.id_mp 
				left join casos on casos.id=cuadro_guia.id_caso
				...
				$ind19="SELECT nna.id, nna.folio, nna.nombre, nna.apellido_p, nna.apellido_m, nna.curp, nna_restituidos.fecha_reg from nna_restituidos, nna where nna_restituidos.fecha_registro BETWEEN '$feci' and '$fecc' and nna_restituidos.respo_reg in($idRespo) and nna.id=nna_restituidos.id_nna";
