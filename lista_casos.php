				...
				<div class="inner"><br> <br> 
					...
					<div class="row uniform">
						<?php if ($_SESSION['spcargo']==3 or $_SESSION['departamento']==16 or $_SESSION['spcargo']==5) { ?>
							...
							<div class="3u">
								<input type="button" value="ALTA" class="button special" onclick="location='reg_caso.php'">
							</div>
						<?php } else { ?>
							<div class="9u">
								<h1>Casos</h1>
						...
						<?php } ?>
						<div class="3u">
							<button onclick="location='casosPendientes.php'">Mis casos pendientes</button>
						</div>
					</div>
					<div class="row uniform">
						<div class="12u">
					...
					<table>
						...
						<tbody>
							...
							$query="SELECT casos.id, casos.folio_c, casos.nombre, casos.descripcion, 
							departamentos.responsable, 
							date_format(casos.fecha_registro, '%d/%m/%Y') as fecha
							from casos left join departamentos on departamentos.id=casos.funcionario_reg 
							WHERE casos.activo=1 and  (departamentos.responsable like '%$buscar%' OR casos.nombre like '%$buscar%' OR casos.fecha like '%$buscar%' OR casos.folio_c like '%$buscar%') 
							order by casos.id desc LIMIT 50";
