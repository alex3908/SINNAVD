...
$fecha_salida2 = date("d/m/Y H:i:s", time());
...
if (!empty($_POST)) {
    ...
    if ($mostrarTipo == 'INICIAL' and $perfil == '1') {
        ...
        $respuesta_aud = mysqli_real_escape_string($mysqli, $_POST['Respuesta']);} else {
        $asunto_aud = null;
        $respuesta_aud = null;
    }
    ...
    if (empty($ate_brindada)) {
        ...
        if ($requisitosAdp) {
            $_SESSION['atenciones'] = $ate_brindada;
            $_SESSION['hora'] = $fecha_salida;
            $_SESSION['asunto'] = $asunto_aud;
            $_SESSION['respuesta'] = $respuesta_aud;
            $_SESSION['telefonica'] = $llamada;

            echo "<script type='text/javascript'>
              	 	location.href = 'entrevista_inicial_adp.php?id=$idHisto';
              	  	</script>";
            ...
            if ($resultUp) {
                $bandera = true;
                echo "<script type='text/javascript'>
                	alert('Visita terminada con exito');
       ...
       <div id="wrapper">
           ...
           <div id="main">
               <div class="inner">
                   ...
                   <form id="registro" enctype="multipart/form-data" name="registro"
...
while ($row = $econsulta->fetch_assoc()) {
    ?>
                                   ...
                                   <div class="6u 12u$(xsmall)">
                                       ...
                                       <input type="checkbox" id="<?php echo 'demo-' . $i; ?>" name="atenci[]"
                                           value="<?php echo $row['atencion']; ?>">
                                       <label for="<?php echo 'demo-' . $i; ?>"><?php echo $row['atencion']; ?></label>
                                   </div>
                                   <?php
$i++;}
if ($mostrarTipo == 'INICIAL' and $perfil == '1') {?>
                               ...
                               <div class=row>
                                   <div class="6u 12u$(xsmall)">Asunto:
                                       <textarea name="Asunto" maxlength="500" rows="5" cols="20"
                                           style="text-transform:uppercase;"
                                           onkeyup="this.value=this.value.toUpperCase();" required></textarea>
                                   </div>

                                   <div class="6u 12u$(xsmall)">Respuesta:
                                       <textarea name="Respuesta" maxlength="500" rows="5" cols="20"
                                           style="text-transform:uppercase;"
                                           onkeyup="this.value=this.value.toUpperCase();" required></textarea>
                                   </div>
                               </div>
                       ...
                       <div class="box">
                           <div class="row uniform">
                               ...
                               <div class="4u 12u$(xsmall)">Fecha salida
                                   <input id="fecha" name="fecha" type="date" min="2020-04-01" max="<?php echo $hoy; ?>"
                                       value="<?php echo $hoy; ?>" disabled>

                               </div>
                               <div class="4u 12u$(xsmall)">Hora de salida
                                   <input id="hora" name="hora" type="text" pattern="[0-9:]{5,5}$"
                                       value="<?php echo $ahora; ?>" disabled>
