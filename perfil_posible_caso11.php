	...
	$qReportes="SELECT reportes_vd.id,  reportes_vd.folio, date_format(reportes_vd.fecha_registro, '%d/%m/%Y %H:%i:%s') as fecha_registro, 
    	...
    	from reportes_vd left join municipios on clm=municipios.id
    	...
    	left join cat_recepcion_reporte on id_recepcion=cat_recepcion_reporte.id 
...
$erepo=$mysqli->query($reportesvd);
	...
	$qCaso="SELECT id_caso, casos.folio_c from casos inner join relacion_pc_caso on id_caso=casos.id
 	 where id_posible_caso='$idPosibleCaso'";
	...
	$qNnaVulnerados="SELECT c.id_nna, id_nna_reportado FROM relacion_nna_nnareportado r
...
<html>
	...
	<body>
		...
		<thead>
			<tr>
			<?php if($idDepartamento==16 and $idPersonal==1) { //control de informacion y administrador lo cambian a acualquier estado-->
			...
			<?php } ?>
						...
						>
					<?php   while ($rowReportes=$rReportes->fetch_assoc()) { $idRep=$rowReportes['id']?>
							<div class=	
							"12u$">
								<div class="box">
									<ul class="alt">
