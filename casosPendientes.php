...
$casosP="SELECT DISTINCT
		...
		FROM
		    casos c 
		        LEFT JOIN
		    nna_caso nc ON c.id = nc.id_caso
		        LEFT JOIN
		    nna ON nc.id_nna = nna.id
		        LEFT JOIN
		    nna_restituidos nr ON nna.id = nr.id_nna
		       
		WHERE
		    c.activo = 1 AND nr.id_nna IS NULL     
		        
		 and (c.funcionario_reg=$idDEPTO or nna.respo_reg=$idDEPTO or c.id in (select DISTINCT c.id from casos c 
		 left join carpeta_inv cr on c.id=cr.id_caso
		 left join cuadro_guia g on g.id_caso=c.id where (cr.respo_reg=$idDEPTO or cr.asignado=$idDEPTO or g.id_sp_registro=$idDEPTO) and g.activo=1 ))";
 $qCasos= $mysqli->query($casosP);
					...
					<div class="row uniform">
						<div class="12u">
							<table>
								...
								<tbody>
									<?php while ($rw= $qCasos->fetch_assoc()) { ?>
										<tr>
											<td><a href="perfil_caso.php?id=<?= $rw['id']?>"><?=$rw['folio_c']?></a></td>
											<td><?=$rw['nombre']?></td>
