<?php	
	...
	$idPc=$_GET['idPc'];
	$idNna=$_GET['idNna'];
	$padres="SELECT padre_fallecido_covid, madre_fallecida_covid from nna_reportados where id=$idNna";
	...
	$registrado = 9; // no funciona el ws de curp
	$persona = $_SESSION['persona']; //toma los datos del nna
	$curp=$_GET['curp'];
	$idPc=$_GET['idPc'];
	$idNna=$_GET['idNna'];
...
$qEstadoNac="SELECT estadosMayus as edoNac, id FROM estados where clave='$lugNac'"; 
	...
	if($registrado!=3 and $registrado!=4){
		...
		casos.id as idC, folio_c  from nna left join nna_caso nc on nc.id_nna=nna.id
				left join casos on casos.id=nc.id_caso
				where (nna.nombre like '%$nombre%' and apellido_p like '%$apellido_p%') or curp like '%$curp%'"; //busca registros similares
		else 
			$qverificarNNA="SELECT nna.id as id_nna, folio, curp, nna.nombre, apellido_p, apellido_m, 
		casos.id as idC, folio_c  from nna left join nna_caso nc on nc.id_nna=nna.id
				left join casos on casos.id=nc.id_caso
				where (nna.nombre like '%$nombre%' and apellido_p like '%$apellido_p%')"; 
            $rverificarNNA=$mysqli->query($qverificarNNA);
            $existeReg=$rverificarNNA->num_rows; }
}
	...
	if(isset($_POST['btnRegistar'])) {  
        ...
        if($padresCovid==2 or $padresCovid==3)
            $madreCovid=1;
		if($registrado==0 or $registrado==4 or $registrado==9){ //sin registros en renapo ni en bd, o con datos incompletos 
			$nombre=$persona['Nombres'];
            $apellido_p=$persona['Apellido1'];
            $apellido_m=$persona['Apellido2'];
            $fechaNac=$persona['FechNac'];
			...
			$qRegistroNna="INSERT INTO nna(folio, nombre, apellido_p, apellido_m,  fecha_nacimiento, sexo, 
                id_estado_nacimiento, lugar_nac, lugar_reg, estado, respo_reg, nna_ex, fecha_registro, id_estado, municipio, localidad, direccion, indigena, afrodescendiente, migrante, id_estado_civil, responna, parentesco, direccion_respo, telefono, observaciones, origen, lugOrigen, violentado, tipoViolencia, llegoAlEstado, padre_fallecido_covid, madre_fallecida_covid) 
                VALUES ('$folio', '$nombre', '$apellido_p', '$apellido_m',  '$fechaNac', '$sexo', 
                '$entiNac', '$txtLugarNacimiento', '$lugarReg', 'P', '$idDEPTO', '0', '$fecha', '$id_estado', '$id_mun', '$id_loc', '$direccion', '$banIndigena', '$banAfro','$banMigra', '$est_civil', '$personaRes', '$parentesco', '$dirRespo', '$telefono', '$observa', '$origen' , '$lugOrigen', '$rViolencia', '$tipoViolencia', $llegoAlEstado, '$padreCovid', '$madreCovid')"; 
			} else {
                ...
                parentesco, direccion_respo, telefono, observaciones, origen, lugOrigen, violentado, tipoViolencia, llegoAlEstado, padre_fallecido_covid, madre_fallecida_covid) 
                ...
                '$entiNac', '$txtLugarNacimiento', '$lugarReg', 'P', '$idDEPTO', '0', '$fecha', '$id_estado', '$id_mun', '$id_loc', '$direccion', '$banIndigena', '$banAfro','$banMigra', '$est_civil',
                 '$personaRes', '$parentesco', '$dirRespo', '$telefono', '$observa', '$origen' , '$lugOrigen', '$rViolencia', '$tipoViolencia', $llegoAlEstado, '$padreCovid', '$madreCovid')"; 
			}
            ...
            if($rRelacion and $rRelacionCaso){		//si se relaaciono bien crea historicos 
				...
				if($rHstDirecciones){
					...
					if($ractDirec){ 
							...
							indigena, afrodescendiente, migrante, origen, lugOrigen, violentado, tipoViolencia, llegoAlEstado) 
							SELECT '$idNnaReg', nombre, apellido_p, apellido_m, curp, '$fechaNacA', sexo, 
							lugar_nac, lugar_reg, '$fecha', '$idDEPTO', indigena, afrodescendiente, migrante, origen, lugOrigen, violentado, tipoViolencia, llegoAlEstado
							...
							if($rRegHstNna and $rRegHisRes){
								if($DocProbatorio=='1'){
									...
									observaciones='$observa', origen='$origen', lugOrigen='$lugOrigen', violentado='$rViolencia', tipoViolencia='$tipoViolencia' , llegoAlEstado= '$llegoAlEstado', padre_fallecido_covid = '$padreCovid', madre_fallecida_covid = '$madreCovid'
									WHERE id = '$idNnaReg'";
								} else {
									$actNna="UPDATE nna SET nombre='$nombre', apellido_p='$apellido_p',
									apellido_m = '$apellido_m', curp= '$curp',fecha_nacimiento = '$fechaNac',
									...
									responna='$personaRes', parentesco='$parentesco', 
									direccion_respo='$dirRespo', telefono='$telefono', 
									observaciones='$observa', origen='$origen', lugOrigen='$lugOrigen', violentado='$rViolencia', tipoViolencia='$tipoViolencia', llegoAlEstado='$llegoAlEstado', padre_fallecido_covid = '$padreCovid', madre_fallecida_covid = '$madreCovid' WHERE id = '$idNnaReg'";
								}
					...
					if($ractDirec){
						if($validada==0){
							...
							if($rRegHstNna){
									...
									afrodescendiente = '$banAfro', migrante='$banMigra', origen='$origen', lugOrigen='$lugOrigen', violentado='$rViolencia', tipoViolencia='$tipoViolencia', llegoAlEstado= '$llegoAlEstado', padre_fallecido_covid = '$padreCovid', madre_fallecida_covid = '$madreCovid'
									WHERE id = '$idNnaReg'";
		   ...
		   if($DocProbatorio=='1'){ //acta de nacimiento
			   ...
			   indigena, afrodescendiente, migrante, responna, parentesco, direccion_respo, observaciones, telefono, origen, lugOrigen, violentado, tipoViolencia, llegoAlEstado, padre_fallecido_covid, madre_fallecida_covid ) 
			   VALUES ('$folio', '$nombre', '$apellido_p', '$apellido_m', '$curp', '$fechaNac', '$sexo', 
			   ...
			   '$banMigra', '$personaRes', '$parentesco', '$dirRespo', '$observa', '$telefono', '$origen' , '$lugOrigen', '$rViolencia', '$tipoViolencia', '$llegoAlEstado', '$padreCovid', '$madreCovid')";
		   } else {
				  ...
				  migrante, responna, parentesco, direccion_respo, observaciones, telefono, origen, lugOrigen, violentado, tipoViolencia, llegoAlEstado, padre_fallecido_covid, madre_fallecida_covid) 
				  VALUES ('$folio', '$nombre', '$apellido_p', '$apellido_m', '$curp', '$fechaNac', '$sexo', 
				  ...
				  '$observa', '$telefono', '$origen' , '$lugOrigen', '$rViolencia', '$tipoViolencia', '$llegoAlEstado', '$padreCovid', '$madreCovid')"; 
		   } 
		...
		<div id="wrapper">
			<div id="main">
				<div class="inner">
					...
					<?php if($registrado==9){ //ws renapo inhabilitado?>
						<h3>¡Por el momento no es posible validar la curp!</h3>
						<h5>Por favor complete la información, la validación se realizará posteriormente</h5>
						<div class="box">
					...
					<?php } else if($registrado==0){ //no se encontro en renapo, ni se encontro la curp en la BD?>
						...
						<div class="box">
							<div class="row uniform">
								<?php if(empty($lugarReg)) { ?>
									...
									<div class="3u">Lugar de registro
									...
									<div class="3u 12u$(xmall"><label>Origen</label>
												<input type="checkbox" id="rbtMigrante" name="rbtMigrante" value="1">
												<label for="rbtMigrante">Migrante</label>
											</div>

									
									<br>
								<?php } else { ?>
								<div class="5u">Lugar de nacimiento
									<input id="lugarNacimiento" name="lugarNacimiento" type="text"  style="text-transform:uppercase;" onkeyup="this.value=this.value.toUpperCase();" <?php if($numName==0) { ?> value="" <?php } else { ?>value="<?=$lugaNac ?>" <?php } ?> >
								</div>
								<div class="5u">Estado civil y/o familiar
									<div class="select-wrapper">
										<select id="edo_civil" name="edo_civil" style="text-transform:uppercase;" onkeyup="this.value=this.value.toUpperCase();" required >
											<option value="">--Seleccione--</option>
											<?php while($rwc = $rEdoCivil->fetch_assoc()){ ?>
								...
								<div class="2u 12u$(xmall"><label>Origen</label>
									<input type="checkbox" id="rbtMigrante" name="rbtMigrante" value="1">
									<label for="rbtMigrante">Migrante</label>
								  </div>
							...
							<div class="row uniform">
								<div class="6u">¿En los últimos tres meses sufrió algún tipo de violencia?
									<div class="select-wrapper">
								...
								<div class="6u">Especifique el tipo principal de violencia sufrida
									<div class="select-wrapper">
