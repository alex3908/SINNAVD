...
<body>
        ...
        <div id="main">
            <div class="inner"><br>
                ...
                <div class="row uniform">
                    ...
                    <div class="2u">
                        <input type="button" class="button special" onclick="location='lista_casos.php'"
                            value="Regresar">
                    </div>
                </div>
                <ul class="alt">
                    <li>
                        <h4>Folio: </h4><?php echo $row['folio_c']; ?>
                    </li>
                    <li>
                        <h4>Reporte: </h4>
    ...
    while ($rwRep = $rReportes->fetch_assoc()) {
        echo $rwRep['folio'] . ", ";
                    ...
                    <li>
                        <h4>Detección: </h4><?php echo $row['descripcion']; ?>
                    </li>
                    <li>
                        <h4>Responsable del registro: </h4><?php echo $row['responsable']; ?>
                    </li>
                    <li>
                        <h4>Fecha: </h4><?php echo $row['fecha']; ?>
                    </li>
                </ul>
                <div class="box">
                        ...
                        <table class="alt">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <?php $idNNA = $row['id'];?>
                                    <td><a
                                            href="perfil_nna.php?id=<?php echo $idNNA; ?>"><?php echo $row['folio']; ?></a>
                                    </td>
    ...
    echo $edad;?>
                                    ...
                                    <td>
                                        ...
                                        <input type="image" alt="Validada" name="validada" src="images/ejecutada.png"
                                            height="30" width="30">
                                        <?php } else {?>
                                        <input type="image" alt="No validada" name="noValidada"
                                            src="images/no_ejecutada.png" height="30" width="30"
                                            onclick="location='validarCurp.php?id=<?=$idNNA?>&T=1'">
                                        <?php }?>
                                    </td>
                                    <td>
                                        <?php $idn = $row['id'];
    ...
    if ($reva > 0) {?>
        ...
        if ($porcentaje >= 60) {
            ...
            ?>
                                        <input name="btnrestituidos" class="button fit small" type="button"
                                            value="nna con derechos restituidos" onclick="if (confirm('¿Restituir derechos?'))
															location='derechos_rest.php?id=<?php echo $idn; ?>&idC=<?php echo $idCaso; ?>'">
                                        <?php } else {?>
                                        <input name="btnCuadroguia" class="button fit small" type="button"
                                            value="Completar cuadro guia"
                                            onclick="location='cuadro_guia.php?id=<?=$idCaso?>'">
                                        <?php }}?>
                                    </td>
                                    <?php if ($idPersonal == 1 and $NumPc == 0) {?>
                                    <td>
                                        <input name="btneliminar" class="button special fit small" type="button"
                                            value="Eliminar"
                                            onclick="location='eliminarnna_caso.php?idnna=<?php echo $row['id']; ?>&idC=<?php echo $idCaso; ?>'">
                                    </td>
                                    <?php }?>
                                </tr>
                                <?php }?>
                        ...
                        <table class="alt">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <?php if ($idPersonal == 1) {?>
                                    <td><input name="eliminar" class="button special fit small" type="button"
                                            value="Eliminar"
                                            onclick="location='eliminarnna_caso.php?idnna=<?php echo $row['id']; ?>&idC=<?php echo $idCaso; ?>'">
                                    </td>
                                    <?php }?>
...
$query = "SELECT carpeta_inv.id, carpeta_inv.nuc, date_format(carpeta_inv.fecha_inicio,'%d/%m/%Y' ) as fecha_ini, casos.folio_c, carpeta_inv.estado, departamentos.responsable
    ...
    while ($row = $resultado->fetch_assoc()) {?>
        ...
        if (is_null($id_asi)) {?>
                                        <?php if ($rows == '0') {?>
                                        <input type="button" class="special button fit small" name="Asignar"
                                            value="Asignar">
                                        <?php } else {?>
                                        <input type="button" class="special button fit small" name="Asignar"
                                            value="Asignar"
                                            onclick="location='asignar_carpeta.php?id=<?php echo $row['id']; ?>'">
                                        <?php }
        } else {?>
        ...
        if ($est == 20) {?>
                                    ...
                                    <td><input type="button" name="Ver" value="Ver"
                                            onclick="location='perfil_carpeta.php?id=<?php echo $row['id']; ?>'">
                                    </td>
                                </tr>
                                <?php }?>
                            </tbody>
                ...
                <div class="row uniform">
                    ...
                    <div class="6u 12u$(xsmall)">
                        <input type="button" name="asignar_curso" class="button fit" value="carpeta de investigación"
                            onclick="location='reg_carpeta.php?id=<?php echo $idCaso; ?>'">
                    </div>
                    <div class="6u 12u$(xsmall)">
                        <input type="button" name="asignar_curso" class="button special fit" value="Añadir NNA"
                            onclick="location='ag_nna_caso.php<?php $_SESSION['idCaso'] = $idCaso;?>'"
                            <?php if ($NumPc > 0) {?> <?php }?>>
                    </div>
                    <?php }?>
                    <div class="6u 12u$(xsmall)">
                        <form id="familia" name="familia" action="<?php $_SERVER['PHP_SELF'];?>" method="POST">
                            <input type="submit" name="cuadro_guia" class="button special fit" value="cuadro guia"
                                onclick="location='cuadro_guia.php?id=<?php echo $idCaso; ?>'">
                        </form>
                    </div>
                    <div class="6u 12u$(xsmall)">
                        <input type="button" name="asignar_curso" class="button fit" value="Editar"
                            onclick="location='editar_caso.php?id=<?php echo $idCaso; ?>'">
                    </div>
