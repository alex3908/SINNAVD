...
$qReportes = "SELECT reportes_vd.id,  reportes_vd.folio, date_format(reportes_vd.fecha_registro, '%d/%m/%Y %H:%i:%s') as fecha_registro,
    	...
    	reportes_vd.respo_reg, reportes_vd.viajaSA, reportes_vd.tipo_migrante
	            ...
	            <div class="inner">
	                ...
	                <form id="familia" name="familia" action="<?php $_SERVER['PHP_SELF'];?>" method="POST">
	                    <div class="row uniform">
	                        ...
	                        <div class="1u 12$(xsmall)">
	                            <input type="button" class="button special small" name="btnImprimir" value="Imprimir"
	                                onclick="location='pdfPosibleCaso.php?idPosibleCaso=<?php echo $idPosibleCaso; ?>'">
	                        </div>
	                    </div>
	                    <br>
	                            ...
	                            <thead>
	                                <tr>
	                                    <td>Juridico:
	                                        ...
	                                        <?php } else {?><a
	                                            href="asignar_posible_caso.php?id=<?php echo $idPosibleCaso; ?>&area=1">Asignar</a>
	                                        <?php }} else {echo $responsableJ;if ($_SESSION['departamento'] == 16 or $idDEPTO == 223 or $permiso > 0) {?>
	                                        <!--solo control de información y erika en su cuenta de administrativo-->
	                                        <input type="submit" class="button small" name="byeJuridico" value="BYE"
	                                            width="30" height="30">
	                                        <?php }}?>
	                                    </td>
	                                    <td>Trabajo Social:
	                                        ...
	                                        <?php } else {?><a
	                                            href="asignar_posible_caso.php?id=<?php echo $idPosibleCaso; ?>&area=2">Asignar</a>
	                                        <?php }} else {echo $responsableTS;if ($permiso > 0) {?>
	                                        <input type="submit" class="button small" name="byeTS" value="BYE" width="30"
	                                            height="30">
	                                        <?php }}?>
	                                    </td>
	                                    <td>Psicologia:
	                                        ...
	                                        <?php } else {?><a
	                                            href="asignar_posible_caso.php?id=<?php echo $idPosibleCaso; ?>&area=3">Asignar</a>
	                                        <?php }} else {echo $responsablePS;if ($permiso > 0) {?>
	                                        <input type="submit" class="button small" name="byePS" value="BYE" width="30"
	                                            height="30">
	                                        <?php }}?>
	                                    </td>
	                                </tr>
...
if ($idDepartamento == 16 and $idPersonal == 1) { //control de informacion y administrador lo cambian a acualquier estado-->
    if ($estadoAtencion == 4) { //pero si ya es atendido positivo checa que tenga un caso
        if ($TieneCaso > 0 and $NumNnaVul >= $numNna) {?>
	                                    <!--Si hay caso vinculado ya no puede cambiarlo-->
	                                    <th><input type="radio" id="NA" name="estado_proceso" value="1"
	                                            onchange="this.form.submit()" disabled>
	                                        <label for="NA">No atendido</label>
	                                    </th>
	                                    <th><input type="radio" id="EP" name="estado_proceso" value="2"
	                                            onchange="this.form.submit()" disabled>
	                                        <label for="EP">En proceso</label>
	                                    </th>
	                                    <th><input type="radio" id="AN" name="estado_proceso" value="3"
	                                            onchange="this.form.submit()" disabled>
	                                        <label for="AN">Atendido negativo</label>
	                                    </th>
	                                    <th><input type="radio" id="AP" name="estado_proceso" value="4"
	                                            onchange="this.form.submit()" checked disabled>
	                                        <label for="AP">Atendido positivo</label>
	                                    </th>
	                                    <th>
	                                        <?php while ($rowCaso = $rCaso->fetch_assoc()) {
            $idCaso = $rowCaso['id_caso'];
            $folioCaso = $rowCaso['folio_c'];?>
	                                        <a href="perfil_caso.php?id=<?php echo $idCaso ?>">
	                                            <?php echo $folioCaso ?></a></br>
	                                        <?php }?>
	                                    </th>
	                                    <?php } else {?>
	                                    <!--sino hay caso lo puede cambiar a cualquier estado-->
	                                    <th><input type="radio" id="NA" name="estado_proceso" value="1"
	                                    ...
	                                    <?php }} else {?>
	                                    <!--si es estado no 4 puede solo checa en que estado es y se puede cambiar a cualquier otro estado-->
	                                    <th><input type="radio" id="NA" name="estado_proceso" value="1"
	                                            onchange="this.form.submit()"
	                                            <?php if ($estadoAtencion == 1 or $estadoAtencion == 0) {?> checked
	                                            disabled <?php }?>>
	                                        <label for="NA">No atendido</label>
	                                    </th>
	                                    <th><input type="radio" id="EP" name="estado_proceso" value="2"
	                                            onchange="this.form.submit()" <?php if ($estadoAtencion == 2) {?> checked
	                                            disabled <?php }?>>
	                                        <label for="EP">En proceso</label>
	                                    </th>
	                                    <th><input type="radio" id="AN" name="estado_proceso" value="3"
	                                            onchange="this.form.submit()" <?php if ($estadoAtencion == 3) {?> checked
	                                            disabled <?php }?>>
	                                        <label for="AN">Atendido negativo</label>
	                                    </th>
	                                    <th><input type="radio" id="AP" name="estado_proceso" value="4"
    ...
    if ($estadoAtencion == 1 or $estadoAtencion == 0) {?>
	                                    <th><input type="radio" id="NA" name="estado_proceso" value="1"
	                                            onchange="this.form.submit()" disabled checked>
	                                        <label for="NA">No atendido</label>
	                                    </th>
	                                    <th><input type="radio" id="EP" name="estado_proceso" value="2"
	                                            onchange="this.form.submit()">
    ...
    if ($estadoAtencion == 1 or $estadoAtencion == 0) {?>
	                                    ...
	                                    <?php if (empty($asignadoPS) and empty($asignadoTS)) {?>
	                                    <!--si no hay asignado de ts o ps puede ponerlo a atendido-->
	                                    <th><input type="radio" id="AN" name="estado_proceso" value="3"
	                                            onchange="this.form.submit()">
	                                        <label for="AN">Atendido negativo</label>
	                                    </th>
	                                    ...
	                                    <?php } else {
        if ($num_acer_psi > 0 or $num_acer_familiar > 0) {?>
	                                    <!-- si hay acercaientos ya puede cambiar a atendido-->
	                                    <th><input type="radio" id="AN" name="estado_proceso" value="3"
	                                            onchange="this.form.submit()">
	                                        <label for="AN">Atendido negativo</label>
	                                    </th>
        ...
        $folioCaso = $rowCaso['folio_c'];?>
	                                    ...
	                                    <th><input type="radio" id="EP" name="estado_proceso" value="2"
	                                            onchange="this.form.submit()" disabled <?php if ($estadoAtencion == 2) {?>
	                                            checked <?php }?>>
	                                        <label for="EP">En proceso</label>
	                                    </th>
	                                    <th><input type="radio" id="AN" name="estado_proceso" value="3"
	                                            onchange="this.form.submit()" disabled <?php if ($estadoAtencion == 3) {?>
	                                            checked <?php }?>>
	                                        <label for="AN">Atendido negativo</label>
	                                    </th>
	                                    <th><input type="radio" id="AP" name="estado_proceso" value="4"
	                                            onchange="this.form.submit()" disabled <?php if ($estadoAtencion == 4) {?>
	                                            checked <?php }?>>
	                                        <label for="AP">Atendido positivo</label>
	                                    </th>
	                                ...
	                                <tr>
	                                    <th colspan="5">Nota: Para cambiar el estado a positivo o negativo primero debe
	                                        cambiar a "En proceso" y existir el registro de al menos un acercamiento en
	                                        caso de que existan personal asignado</th>
	                                </tr>
	                                <?php }?>
	                            </thead>
	                        </table>
    ...
    $can = $exv->num_rows;
    if ($can > 0) {?>
	                        <input class="button special fit small" type="button" name="" value="acercamientos"
	                            onclick="location='registro_nna_ac.php?id=<?php echo $idPosibleCaso; ?>'">
	                        <?php } else {
        if ($asignadoPS == $idDEPTO or $asignadoTS == $idDEPTO or $_SESSION['departamento'] == 16) {?>
	                        <input class="button special fit small" type="button" name="" value="Registrar acercamientos"
	                            onclick="location='registro_nna_ac.php?id=<?php echo $idPosibleCaso; ?>'">
	                        <?php }}
}?>
	                    </div>
	                    <div class="box" align="center">
	                        <table>
	                            <thead>
	                                <td colspan="6">NNA registrados: </td>
	                                <td><input type="button" class="button special small" name="Add" value="Añadir"
	                                        onclick="location='agregarNnaRep.php?idPosibleCaso=<?=$idPosibleCaso?>'">
	                                </td>
	                                <tr>
	                                    <td><b>Nombre</b></td>
	                                    <td><b>Sexo </b></td>
    ...
    ?> </td>
        ...
        echo "Padre";
    } elseif ($row['padre_fallecido_covid'] == 0 and $row['madre_fallecida_covid'] == 1) {
        echo "Madre";
    } elseif ($row['padre_fallecido_covid'] == 1 and $row['madre_fallecida_covid'] == 1) {
        echo "Ambos";
    }
    ?>
	                                    <td><?php echo $row["edad"]; ?> </td>
	                                    <?php if ($numName == 0) {
        if ($idDEPTO == $asignadoJ or $idDEPTO == $asignadoPS or $idDEPTO == $asignadoTS or $idDEPTO == $ResponRegistro or ($idDepartamento == 16 and $idPersonal == 1) or $idb = 70) {?>
	                                    <td><a
	                                            href="editar_nna_reportado.php?id=<?php echo $row['idNna']; ?>&idPosCaso=<?php echo $idPosibleCaso; ?>">Editar</a>
	                                    </td>
	                                    <td>
	                                        <?php if (empty($relacion)) {?>
	                                        <a
	                                            href="borrar_nna_reportado_pc.php?id=<?php echo $row['idNna']; ?>">Eliminar</a>
	                                        <?php } else {?>
...
while ($rowReportes = $rReportes->fetch_assoc()) {$idRep = $rowReportes['id']?>
	                            ...
	                            <ul class="alt">
	                                <div class="row uniform">
	                                    <div class="4u 12u$(small)">
	                                        <li>
	                                            <h4>Folio del reporte:
	                                                <?php echo $rowReportes['folio']; ?></h4>
	                                        </li>
	                                        <li>
	                                            <h4>Persona que reporto: </h4>
	                                            <?php echo $rowReportes['persona_reporte']; ?>
	                                        </li>
	                                    </div>
	                                    <div class="3u 12u$(small)">
	                                        <li>
	                                            <h4>Fecha: <?php echo $rowReportes['fecha_registro']; ?> </h4>
	                                        </li>
	                                        <li>
	                                            <h4>Tipo de maltrato: <?php echo $rowReportes['maltrato']; ?> </h4>
	                                        </li>
	                                    </div>
	                                    <div class="5u 12u$(small)">
	                                        <li>
	                                            <h4>Forma de recepcion: <?php echo $rowReportes['recepcion']; ?> </h4>
	                                        </li>
	                                        <li>
	                                            <h4>Distrito: <?php echo $rowReportes['distrits']; ?> </h4>
	                                        </li>
	                                    </div>
	                                </div>
	                                <div class="row uniform">
	                                    <div class="6u 12u$(small)">
	                                        <li>
	                                            <h4>Narración de lo sucedido: </h4><?php echo $rowReportes['narracion']; ?>
	                                        </li>
	                                        <li>
	                                            <h4>Otros datos u observaciones relevantes: </h4>
	                                            <?php echo $rowReportes['otros_datos']; ?>
	                                        </li>
	                                        <?php if ($rowReportes['tipo_migrante'] && $rowReportes['viajaSA']) {?>
	                                        <li>


	                                            <h4>Datos Migracion</h4>
	                                            <?php echo $rowReportes['tipo_migrante']; ?>
	                                            <br>
	                                            <?php echo $rowReportes['viajaSA']; ?>
	                                    ...
	                                    <div class="6u 12u$(small)">
	                                        <li>
	                                            <h4>Ubicación: </h4>
	                                            <?php echo 'Municipio ' . $rowReportes['municipio'] . '<br>Localidad ' . $rowReportes['localidad'] . '<br>Calle ' . $rowReportes['calle'] . '<br>Referencias ' . $rowReportes['ubicacion']; ?>
	                                        </li>
	                                        <li>
	                                            <h4>Responsable de registro: </h4>
	                                            <?php echo $rowReportes['responsable']; ?>
	                                        </li>
	                                    </div>
	                                    <?php if ((($idDEPTO == $asignadoJ or $idDEPTO == $asignadoTS or $idDEPTO == $asignadoPS or $idDEPTO == $rowReportes['respo_reg']) or ($idDepartamento == '16' and $idPersonal == '1') or $idb = 70)) {?>
	                                    <div class="12u">
	                                        <a
	                                            href="editar_reporte.php?idRep=<?=$idRep?>&idPc=<?=$idPosibleCaso?>">Editar</a>
	                                    </div>
	                                    <?php }?>
	                            </ul>
	                        </div>
	                ...
	                <div class="row uniform">
	                    <div class="12u">
	                        <input type="button" class="button special small" name="btnHistorial" value="Ver Historial"
	                            onclick="location='historial_pc.php?id=<?php echo $idPosibleCaso; ?>'">
	                    </div>
